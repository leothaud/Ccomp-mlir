...
public:
  ReplaceGenericPattern(mlir::MLIRContext *context)
      : mlir::RewritePattern(
            ccomp::myCast::GenericExpressionOp::getOperationName(), 1,
            context) {}
  ...
  matchAndRewrite(mlir::Operation *op,
    ...
    auto exprType = llvm::cast<MyCastInterface>(genOp.getExpr().getDefiningOp())
                        .getTypeAttr();
    std::optional<mlir::OpResult> defaultExpr;
    for (auto case_ : genOp.getCases()) {
      if (auto typeCase =
              llvm::dyn_cast<TypeGenericItemOp>(case_.getDefiningOp())) {
        if (areEquals(
                llvm::cast<MyCastInterface>(typeCase.getCond().getDefiningOp())
                    .getTypeAttr(),
                exprType)) {
          rewriter.replaceAllUsesWith(
              genOp, typeCase.getBody().getDefiningOp()->getResult(0));
          rewriter.eraseOp(genOp);
          return mlir::success();
        }
      } else if (auto defaultCase = llvm::dyn_cast<DefaultGenericItemOp>(
                     case_.getDefiningOp())) {
